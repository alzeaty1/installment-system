...
def _apply_payment(contract, amount, pay_date, payment_method="", receiver=None, received_by="", sender_account="", sender_name="", transfer_image=None, notes="", target_installment=None):
    """سجّل الدفعة على القسط المفتوح فقط — بدون أي توزيع تلقائي.

    الزيادة عن قيمة القسط تُسجل على نفس القسط (حالة "زيادة").
    ...
    inst = target_installment
    # الفورم ModelForm مربوط بنفس الكائن، و is_valid() بيكتب cleaned_data على
    # instance — فقيمة paid_amount الحالية في الذاكرة ممكن تكون اتلغمت بالقيمة
    # الجديدة. نرجّع للقيمة المحفوظة فعليًا في الداتابيز قبل الإضافة.
    db_paid = Installment.objects.filter(pk=inst.pk).values_list("paid_amount", flat=True).first() or Decimal("0")
    inst.paid_amount = db_paid + amount
    ...
    if inst.status != Installment.STATUS_CLOSED:
...
def installment_pay(request, id):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        touched_numbers = f"#{installment.installment_number}"
        _log_activity(
            request,
            action=ActivityLog.ACTION_PAYMENT,
            model_name="Installment",
            obj=last_inst or installment,
            description=f"تسجيل دفع {payment_amount} جنيه على العقد {contract.contract_number} للعميل {contract.customer.name} — على القسط {touched_numbers}",
